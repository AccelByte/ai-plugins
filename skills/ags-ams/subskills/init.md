...
- Each stage is implemented by another subskill. Read `subskills/{stage}.md` before running it.
...
Read `subskills/account.md` and follow it. When done:
...
Read `subskills/sdk.md` and follow it.

When done, confirm the integration checklist from `sdk.md` is satisfied before proceeding.

If the SDK is not installed (Unreal/Unity SDK missing), stop and direct to `/ags install-unreal-sdk` or `/ags install-unity-sdk`.
...
If yes → read `subskills/debug.md` and follow Mode 1 (AMS Simulator). After testing, continue.
...
Read `subskills/upload.md` and follow it. Capture the uploaded image name.
...
Read `subskills/fleet.md` and follow it. At minimum, produce a fleet summary with claim keys.
...
Read `subskills/session.md` and follow it. The fleet claim key from Stage 4 feeds directly into the session template.
...
| SDK not installed | Stop at Stage 2. Direct to `/ags install-unreal-sdk` or `/ags install-unity-sdk` then re-run `/ags-ams sdk`. |
