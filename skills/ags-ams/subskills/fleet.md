...
Guide a developer through creating and configuring AMS fleets — fleet type selection, instance type, scaling parameters, timeouts, regions, and claim keys. Fleet configuration lives in the Admin Portal (no CLI commands); this subskill walks the configuration fields and helps the developer make the right choices.
...
- Fleet configuration is done through the Admin Portal, not the CLI. This subskill produces configuration guidance and values the developer enters manually, not automated commands.
...
- No `Bash` — fleet config is Admin Portal-only.
