...
- Criteria values are `"distance"` and `"exact"` only (from the reference). Do not invent other criteria.
...
- Matching rules: one `matching_rule` entry per attribute criterion. Use `"distance"` for numeric ranges (MMR, score). Use `"exact"` for discrete categories (game mode, region). Set `isForBalancing: true` for any attribute that should influence rebalance.
...
| User asks for >20 matching_rule entries | Note the limit (20 per the reference); suggest consolidating criteria. |
