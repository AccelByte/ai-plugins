...
**DS states:** Creating → Ready → In Session → Draining → Unresponsive
...
**On-demand DS start** — dev fleet behavior: when a build config is claimed for the first time, AMS starts a DS on demand. The first claim always fails; AGS Session retries after 60 seconds.
