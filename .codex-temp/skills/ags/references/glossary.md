...
**Region.** Geographic deployment. AGS public cloud runs in multiple regions; private cloud is single-region by default. Matchmaking and AMS use region for latency optimization.

**Public Cloud** *(formerly **Shared Cloud**)*. The default deployment tier — AccelByte-managed, multi-tenant. Renamed; the old name still appears in older AccelByte documentation, screenshots, and support threads, and means exactly this. Distinct from Private Cloud (single-tenant, AccelByte-managed) and BYOC (your infrastructure). See `../deployment/public-cloud.md`.
